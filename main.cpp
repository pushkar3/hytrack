	...
	for(int i = 0; i < 1000; i++) {
		...
		if (!image.empty()) {
			...
			if(trackObject) {
				...
				//rectangle(image, Point(win1.x, win1.y), Point(win1.x + win1.width, win1.y + win1.height), Scalar(255, 0, 0), 2, CV_AA);
				//rectangle(image, Point(win2.x, win2.y), Point(win2.x + win2.width, win2.y + win2.height), Scalar(0, 0, 255), 2, CV_AA);
